...
# NEW: Import for streaming output base class
from langchain.callbacks.streaming_stdout import StreamingStdOutCallbackHandler

# NEW: Custom Callback Handler for delayed streaming
class DelayedStreamingStdOutCallbackHandler(StreamingStdOutCallbackHandler):
    """Callback handler for streaming. Adds a delay."""
    def __init__(self, delay_sec: float = 0.6): # Adjust delay_sec to your preference (e.g., 0.05 for slower)
        super().__init__()
        self.delay_sec = delay_sec

    def on_llm_new_token(self, token: str, **kwargs) -> None:
        """Run on new LLM token. Only prints if the token is not an empty string."""
        if token: # Only print if there's actual content
            print(token, end='', flush=True)
            time.sleep(self.delay_sec) # Introduce a small delay here
...
azure_file_share_path = os.getenv("AZURE_FILE_SHARE_PATH", "")
...
# 4. Initialize LLM and RAG Chain (MODIFIED for streaming with delay)
def initialize_rag_chain(vectorstore):
    print("Initializing LLM and RAG chain using Azure OpenAI with delayed streaming...")
    llm = AzureChatOpenAI(
        ...
        callbacks=[DelayedStreamingStdOutCallbackHandler(delay_sec=0.02)] # Use our custom handler with a delay
    )
...
if __name__ == "__main__":
    # --- Configuration ---
    local_knowledge_base_path = r"knowledge_base.txt"
    # local_knowledge_base_path = r"C:\Users\aebrah51\Desktop\YourLocalDocsFolder"
    ...
    while True:
        ...
        print("\n--- Answer ---")
        # The answer will now stream directly to stdout due to the callback.
        # The 'response' object will still contain the full result once it's done.
